...
const f6 = () => {
  let set = new Set(a6);
  document.querySelector(`.out-6`).textContent = set.size;
};
...
const f7 = () => {
  const userPass = document.querySelector(`.i-7`).value;
  if (userPass.length > 6) {
    const sPassArr = Array.from(userPass);
    const set = new Set(sPassArr);
    if (sPassArr.length == set.size) {
...
const f10 = (out_set, elem) => {
  document.querySelector(elem).innerHTML = Array.from(out_set);
};
...
const f13 = () => {
  ...
  let set = new Set(filteredArr);
  let obj = {};
  for (let elemFiltered of set) {
    let counter = 0;
    for (let elem of filteredArr) {
      if (elem == elemFiltered) {
        counter++;
      }
    }
    obj[elemFiltered] = counter;
  }
  console.log(obj);
};
