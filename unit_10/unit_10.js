...
let ar6 = [`anton`, `fish`, `03`, `march`];

function f6() {
  let out = ``;
  for (const iterator of ar6) {
    out += iterator + ` `;
  }
...
function f16() {
  let out_even = ``;
  let out_odd = ``;
  for (const iterator of ar16) {
    if (iterator % 2 == 0) {
      ar16_even.push(iterator);
    } else {
      ar16_odd.push(iterator);
    }
  }
  out_even = ar16_even.join(` `);
  out_odd = ar16_odd.join(` `);
