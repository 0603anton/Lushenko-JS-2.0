...
function f6() {
  ...
  for (const iterator of a6) {
    for (const subEle of iterator) {
      if (subEle % 2 != 0) {
        out += subEle;
      }
    }
...
function f13() {
  for (let i = 0; i < 8; i++) {
    a13[i] = [];
    for (let j = 0; j < 8; j++) {
      // console.log(flag + ` flag do proverky + nomer stroky ${i}`);
      if (j == 0) flag = !flag;

      if (flag) a13[i].push(0);
      else a13[i].push(1);
      // console.log(flag + ` flag posle proverky`);
      flag = !flag;
      // console.log(flag + ` flag posle proverky + posle izmenenia v konce`);
    }
  }
  console.log(a13);
}
