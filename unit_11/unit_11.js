...
function f11() {
  ...
  let isNumber = d11.indexOf(input11);
  document.querySelector(`.out-11`).innerHTML = isNumber;
}
...
function f12() {
  ...
  let randomNumber = +document.querySelector(`.i-12`).value;
...
function f16() {
  let newArr = d16.concat(d161, d162);

  showArr(".out-16", newArr);
}
...
function f17() {
  for (let elem of d171) {
    d17.push(elem);
  }
  for (let elem of d172) {
    d17.push(elem);
  }
  showArr(".out-17", d17);
}
...
function f18() {
  ...
  document.querySelector(`.out-18`).textContent = d18.includes(input18);
}
